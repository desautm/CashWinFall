...
- On tire simultanément $n$ boules dans une urne contenant $N_1=pN$ boules gagnantes et $N_2=qN$ boules perdantes (avec $q=1-p$). On compte alors le nombre de boules gagnantes extraites.

- $P(X=x) = \dfrac{\binom{pN}{x}\binom{qN}{n-x}}{\binom{N}{n}}$
...
- C'est-à-dire qu'il comporte le plus petit nombre de points et de droites, à savoir 7 de chaque. 
...
- On doit donc détruire la symétrie de la liste de Denniston en enlevant tous les billets contenant un 47 ou un 48.
...
## CashWinFall et Denniston

```{r, random-number-seed, echo=FALSE}
set.seed(42)
wticket <- sort(sample(46,6))

t1 <- wticket[c(1,2,3,4,5)]
t2 <- wticket[c(1,2,3,4,6)]
t3 <- wticket[c(1,2,3,5,6)]
t4 <- wticket[c(1,2,4,5,6)]
t5 <- wticket[c(1,3,4,5,6)]
t6 <- wticket[c(2,3,4,5,6)]
```

- Supposons que le billet gagnant est le `r wticket`.
...
- Avez-vous un billet avec `r t1`? *Un* des billets de Denniston possède cette combinaison.

- La seule manière de ne pas l'avoir est que le billet de Denniston était `r c(t1,47)` ou `r c(t1,48)`.

- Mais qu'en est-il de la combinaison `r t2`? Si 47 faisait partie de `r c(t1,47)`, il est impossible que `r c(t2,47)` fasse partie de la liste de Denniston, car vous auriez deux billets avec la même combinaison de cinq numéros.

- Le raisonnement est identique avec 48.
...
```{r, proba-denniston}
pjackpot <- nrow(cashwinfall)/choose(46,6)*100
p4 <- ((n47-n47_48)/nrow(denniston))^2*100
p5 <- (n47+n48-n47_48)/nrow(denniston)*100
p6 <- 100-p4-p5-pjackpot
```

Si vous acheter les `r format(nrow(cashwinfall),big.mark = " ")` billets de Denniston vous avez:

`r pjackpot`% de gagner le gros lot

`r p6`% de gagner six des combinaisons 5 dans 6

`r p5`% de gagner cinq des combinaisons 5 dans 6

`r p4`% de gagner quatre des combinaisons 5 dans 6
